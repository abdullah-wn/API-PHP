...
class Less_Parser
{
    ...
    public function getCss()
    {
        ...
        try {
            ...
            if (Less_Parser::$options['sourceMap']) {
                $generator = new Less_SourceMap_Generator(
                    $evaldRoot,
                    Less_Parser::$contentsMap,
                    Less_Parser::$options
                );
                // will also save file
    ...
    private function PostVisitors($evaldRoot)
    {
        ...
        if (Less_Parser::$options['plugins']) {
            foreach (Less_Parser::$options['plugins'] as $plugin) {
                if (
                    property_exists($plugin, 'isPreEvalVisitor') &&
                    $plugin->isPreEvalVisitor
                ) {
                    continue;
                }

                if (
                    property_exists($plugin, 'isPreVisitor') &&
                    $plugin->isPreVisitor
                ) {
    ...
    public function parseFile($filename, $uri_root = '', $returnRoot = false)
    {
        ...
        if (
            !$returnRoot &&
            !empty($uri_root) &&
            basename($uri_root) == basename($filename)
        ) {
    ...
    public function SetFileInfo($filename, $uri_root = '')
    {
        ...
        if (isset($this->env->currentFileInfo)) {
            $currentFileInfo['entryPath'] =
                $this->env->currentFileInfo['entryPath'];
            $currentFileInfo['entryUri'] =
                $this->env->currentFileInfo['entryUri'];
            $currentFileInfo['rootpath'] =
                $this->env->currentFileInfo['rootpath'];
        } else {
        ...
        //inherit reference
        if (
            isset($this->env->currentFileInfo['reference']) &&
    ...
    public function SetCacheDir($dir)
    {
        if (!file_exists($dir)) {
            ...
            throw new Less_Exception_Parser(
                'Less.php cache directory couldn\'t be created: ' . $dir
            );
        } elseif (!is_dir($dir)) {
            throw new Less_Exception_Parser(
                'Less.php cache directory doesn\'t exist: ' . $dir
            );
        } elseif (!is_writable($dir)) {
            throw new Less_Exception_Parser(
                'Less.php cache directory isn\'t writable: ' . $dir
            );
        } else {
    ...
    private function GetRules($file_path)
    {
        ...
        if ($this->pos < $this->input_len) {
            throw new Less_Exception_Chunk(
                $this->input,
                null,
                $this->furthest,
            ...
            if (Less_Parser::$options['cache_method'] == 'callback') {
                ...
                switch (Less_Parser::$options['cache_method']) {
                    ...
                    case 'php':
                        file_put_contents(
                            $cache_file,
                            '<?php return ' . self::ArgString($rules) . '; ?>'
                        );
                        break;
                    case 'var_export':
    ...
    public function CacheFile($file_path)
    {
        if ($file_path && $this->CacheEnabled()) {
            ...
            return Less_Cache::$cache_dir .
                Less_Cache::$prefix .
                base_convert(sha1(json_encode($parts)), 16, 36) .
                '.lesscache';
        }
    }
    ...
    {
        return strpos(" \t\n\r\v\f", $this->input[$this->pos + $offset]) !==
            false;
    }
    ...
    public function PeekChar($tok)
    {
        ...
        return $this->pos < $this->input_len &&
            $this->input[$this->pos] === $tok;
    }
    ...
    private function FromKeyword($keyword)
    {
        ...
        if (Less_Colors::hasOwnProperty($keyword)) {
            ...
            return $this->NewObj1(
                'Less_Tree_Color',
                substr(Less_Colors::color($keyword), 1)
            );
        }
    ...
    private function parseEntitiesCall()
    {
        ...
        if ($name) {
            return $this->NewObj4('Less_Tree_Call', [
                $name,
                $args,
                $index,
                $this->env->currentFileInfo,
    ...
    private function parseEntitiesVariable()
    {
        ...
        if (
            ...
            return $this->NewObj3('Less_Tree_Variable', [
                $name[0],
                $index,
                $this->env->currentFileInfo,
            ]);
    ...
    private function parseEntitiesDimension()
    {
        ...
        if ($value) {
            if (isset($value[2])) {
                return $this->NewObj2('Less_Tree_Dimension', [
                    $value[1],
                    $value[2],
                ]);
            }
    ...
    private function parseEntitiesJavascript()
    {
        ...
        $str = $this->MatchReg('/\\G`([^`]*)`/');
        if ($str) {
            return $this->NewObj3('Less_Tree_Javascript', [
                $str[1],
                $this->pos,
                $e,
            ]);
    ...
    private function parseElement()
    {
        ...
        if (!is_null($e)) {
            return $this->NewObj4('Less_Tree_Element', [
                $c,
                $e,
                $index,
                $this->env->currentFileInfo,
    ...
    private function parseSelector($isLess = false)
    {
        ...
        while (
            ($isLess && ($extend = $this->parseExtend())) ||
            ($isLess && ($when = $this->MatchReg('/\\Gwhen/'))) ||
            ($e = $this->parseElement())
        ) {
            if ($when) {
                $condition = $this->expect(
                    'parseConditions',
                    'expected condition'
                );
            } elseif ($condition) {
    ...
    {
        return ($tag = $this->MatchReg('/\\G[A-Za-z][A-Za-z-]*[0-9]?/'))
            ? $tag
            : $this->MatchChar('*');
    }
    ...
    private function parseRule($tryAnonymous = null)
    {
        ...
        if ($name) {
            ...
            if (!$value) {
                ...
                if (
                    !$tryAnonymous &&
                    (Less_Parser::$options['compress'] || $isVariable)
                ) {
    ...
    function parseMultiplication()
    {
        ...
        if ($return) {
            while (true) {
                ...
                $m->parensInOp = true;
                $a->parensInOp = true;
                $return = $this->NewObj3('Less_Tree_Operation', [
                    $op,
                    [$return, $a],
                    $isSpaced,
                ]);
    ...
    private function parseConditions()
    {
        ...
        if ($a) {
            while (true) {
                ...
                $return = $this->NewObj4('Less_Tree_Condition', [
                    'or',
                    $return,
                    $b,
                    $index,
    ...
    private function parseCondition()
    {
        ...
        if ($a) {
            ...
            if ($op) {
                ...
                if ($b) {
                    $c = $this->NewObj5('Less_Tree_Condition', [
                        $op[0],
                        $a,
                        $b,
                        $index,
            ...
            $this->expectChar(')');
            return $this->MatchReg('/\\Gand/')
                ? $this->NewObj3('Less_Tree_Condition', [
                    'and',
                    $c,
                    $this->parseCondition(),
                ])
    ...
    public function NewObj1($class, $arg)
    {
        ...
        if ($this->CacheEnabled()) {
            $obj->cache_string =
                ' new ' . $class . '(' . Less_Parser::ArgString($arg) . ')';
        }
        return $obj;
    }
    ...
    public static function ArgString($arg)
    {
        ...
        if ($type === 'object') {
            ...
            foreach ($arg as $k => $a) {
                $string .=
                    var_export($k, true) . ' => ' . self::ArgString($a) . ',';
            }
            return $string . ')';
        }
    ...
    public static function AbsPath($path, $winPath = false)
    {
        if (
            strpos($path, '//') !== false &&
            preg_match('_^(https?:)?//\\w+(\\.\\w+)+/\\w+_i', $path)
        ) {
    ...
    {
        return Less_Parser::$options['cache_method'] &&
            (Less_Cache::$cache_dir ||
                Less_Parser::$options['cache_method'] == 'callback');
    }
}
