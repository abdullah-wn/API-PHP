...
class Less_Cache
{
    ...
    public static function Get(
        ...
        $list_file =
            Less_Cache::$cache_dir . Less_Cache::$prefix . $hash . '.list';

        // check cached content
        if (
            !isset($parser_options['use_cache']) ||
    ...
    private static function CompiledName($files, $extrahash)
    {
        ...
        return Less_Cache::$prefix .
            sha1(json_encode($temp) . $extrahash) .
            '.css';
    }
        ...
        if (!file_exists(Less_Cache::$cache_dir)) {
            if (!mkdir(Less_Cache::$cache_dir)) {
                throw new Less_Exception_Parser(
                    'Less.php cache directory couldn\'t be created: ' .
                        Less_Cache::$cache_dir
                );
            }
        } elseif (!is_dir(Less_Cache::$cache_dir)) {
            throw new Less_Exception_Parser(
                'Less.php cache directory doesn\'t exist: ' .
                    Less_Cache::$cache_dir
            );
        } elseif (!is_writable(Less_Cache::$cache_dir)) {
            throw new Less_Exception_Parser(
                'Less.php cache directory isn\'t writable: ' .
                    Less_Cache::$cache_dir
            );
        }
    }
