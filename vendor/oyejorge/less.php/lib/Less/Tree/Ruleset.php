...
class Less_Tree_Ruleset extends Less_Tree
{
    ...
    public function compile($env)
    {
        ...
        for ($i = 0; $i < $rsRuleCnt; $i++) {
            if (
                $ruleset->rules[$i] instanceof Less_Tree_Mixin_Definition ||
                $ruleset->rules[$i] instanceof Less_Tree_DetachedRuleset
            ) {
    ...
    function makeImportant()
    {
        ...
        foreach ($this->rules as $rule) {
            if (
                ...
                $rule instanceof Less_Tree_Ruleset ||
                $rule instanceof Less_Tree_NameValue
            ) {
    ...
    public function matchCondition($args, $env)
    {
        ...
        if (
            ...
            !$lastSelector->condition->compile($env->copyEvalEnv($env->frames))
        ) {
            return false;
        }
        return true;
    ...
    public function genCSS($output)
    {
        ...
        if (!Less_Parser::$options['compress']) {
            if (Less_Environment::$tabLevel) {
                $tabRuleStr =
                    "\n" .
                    str_repeat(
                        Less_Parser::$options['indentation'],
                        Less_Environment::$tabLevel
                    );
                $tabSetStr =
                    "\n" .
        ...
        if (!$this->root) {
			...
			*/
            ...
            $output->add(
                (Less_Parser::$options['compress'] ? '{' : " {") . $tabRuleStr
            );
        }
        ...
        for ($i = 0; $i < $ruleNodes_len; $i++) {
            ...
            if (
                $i + 1 === $ruleNodes_len &&
                (!$this->root || $rulesetNodes_len === 0 || $this->firstRoot)
            ) {
    ...
    public function joinSelector(&$paths, $context, $selector)
    {
        ...
        foreach ($selector->elements as $el) {
            ...
            if ($el->value !== '&') {
                ...
                if ($currentElements) {
                    $this->mergeElementsOnToSelectors(
                        $currentElements,
                        $newSelectors
                    );
                }
                ...
                foreach ($newSelectors as $sel) {
                    ...
                    if (!$context) {
                        ...
                        // it is not lost
                        if ($sel) {
                            $sel[0]->elements = array_slice(
                                $sel[0]->elements,
                                0
                            );
                            $sel[0]->elements[] = new Less_Tree_Element(
                                $el->combinator,
                                '',
                                $el->index,
                                $el->currentFileInfo
                        ...
                        foreach ($context as $parentSel) {
                            ...
                            if (count($parentSel) > 1) {
                                $afterParentJoin = array_merge(
                                    $afterParentJoin,
                                    array_slice($parentSel, 1)
                                );
                            }
                            ...
                            if ($parentSel) {
                                ...
                                $newJoinedSelector->elements[] = new Less_Tree_Element(
                                    $el->combinator,
                                    $parentSel[0]->elements[0]->value,
                                    $el->index,
                                    $el->currentFileInfo
                                );
    ...
    function mergeElementsOnToSelectors($elements, &$selectors)
    {
        ...
        foreach ($selectors as &$sel) {
            ...
            if ($sel) {
                $last = count($sel) - 1;
                $sel[$last] = $sel[$last]->createDerived(
                    array_merge($sel[$last]->elements, $elements)
                );
            } else {
