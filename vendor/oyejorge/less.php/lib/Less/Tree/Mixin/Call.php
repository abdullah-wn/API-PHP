                ...
                if ($mixin->matchArgs($args, $env)) {
                    ...
                    if ($mixin instanceof Less_Tree_Ruleset) {
                        for ($f = 0; $f < 2; $f++) {
                            ...
                            $conditionResult[$f] = $mixin->matchCondition(
                                $args,
                                $env
                            );
                        }
                ...
                if ($candidate === $defNone || $candidate === $defaultResult) {
                    try {
                        ...
                        if (!($mixin instanceof Less_Tree_Mixin_Definition)) {
                            $mixin = new Less_Tree_Mixin_Definition(
                                '',
                                [],
                                $mixin->rules,
                                null,
                            ...
                            $mixin->originalRuleset =
                                $mixins[$m]->originalRuleset;
                        }
                        $rules = array_merge(
                            $rules,
                            $mixin->evalCall($env, $args, $this->important)
                                ->rules
                        );
                    } catch (Exception $e) {
    ...
    private function IsRecursive($env, $mixin)
    {
        foreach ($env->frames as $recur_frame) {
            if (!($mixin instanceof Less_Tree_Mixin_Definition)) {
                ...
                if (
                    isset($recur_frame->originalRuleset) &&
                    $mixin->ruleset_id === $recur_frame->originalRuleset
                ) {
                    return true;
                }
            }
