...
class Less_Tree_Mixin_Definition extends Less_Tree_Ruleset
{
    ...
    public function __construct(
        ...
        $this->name = $name;
        $this->selectors = [
            new Less_Tree_Selector([new Less_Tree_Element(null, $name)]),
        ];
            ...
            for ($i = 0; $i < $argsLength; $i++) {
                ...
                if ($arg && $arg['name']) {
                    ...
                    foreach ($params as $j => $param) {
                        if (
                            !isset($evaldArguments[$j]) &&
                            $arg['name'] === $params[$j]['name']
                        ) {
        ...
        foreach ($params as $i => $param) {
            ...
            if (isset($param['name']) && $param['name']) {
                if (isset($param['variadic'])) {
                    ...
                    array_unshift(
                        $frame->rules,
                        new Less_Tree_Rule($param['name'], $val)
                    );
    ...
    public function evalCall($env, $args = null, $important = null)
    {
        ...
        array_unshift(
            $frame->rules,
            new Less_Tree_Rule('@arguments', $ex->compile($env))
        );
    ...
    public function matchCondition($args, $env)
    {
        ...
        $frame = $this->compileParams(
            $env,
            array_merge($this->frames, $env->frames),
            $args
        );
    ...
    public function matchArgs($args, $env = null)
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            if (
                !isset($this->params[$i]['name']) &&
                !isset($this->params[$i]['variadic'])
            ) {
                if (
                    $args[$i]['value']->compile($env)->toCSS() !=
                    $this->params[$i]['value']->compile($env)->toCSS()
                ) {
                    return false;
                }
            }
