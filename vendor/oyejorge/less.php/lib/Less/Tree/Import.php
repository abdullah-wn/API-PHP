...
class Less_Tree_Import extends Less_Tree
{
    ...
    {
        return new Less_Tree_Import(
            $this->path->compile($env),
            $this->features,
            $this->options,
            $this->index,
    ...
    public function compilePath($env)
    {
        ...
        if (!($path instanceof Less_Tree_URL)) {
            if ($rootpath) {
                ...
                if (
                    $pathValue &&
                    Less_Environment::isPathRelative($pathValue)
                ) {
                    $path->value =
                        $this->currentFileInfo['uri_root'] . $pathValue;
                }
            }
    ...
    public function compile($env)
    {
        ...
        if (is_callable(Less_Parser::$options['import_callback'])) {
            $path_and_uri = call_user_func(
                Less_Parser::$options['import_callback'],
                $evald
            );
        }
        ...
        if (
            isset($this->options['optional']) &&
            $this->options['optional'] &&
            !file_exists($full_path) &&
            (!$evald->css || !empty($this->currentFileInfo))
        ) {
            return [];
        }
        ...
        // css ?
        if ($evald->css) {
            $features = $evald->features
                ? $evald->features->compile($env)
                : null;
            return new Less_Tree_Import(
                $this->compilePath($env),
                $features,
                $this->options,
                $this->index
            );
    ...
    public function PathAndUri()
    {
        ...
        if ($evald_path) {
            ...
            if (Less_Environment::isPathRelative($evald_path)) {
                ...
                $import_dirs[$this->currentFileInfo['currentDirectory']] =
                    $this->currentFileInfo['uri_root'];
            } else {
                //otherwise, the file should be relative to the server root
                $import_dirs[$this->currentFileInfo['entryPath']] =
                    $this->currentFileInfo['entryUri'];
            ...
            foreach ($import_dirs as $rootpath => $rooturi) {
                if (is_callable($rooturi)) {
                    ...
                    if (file_exists($path)) {
                        ...
                        $uri = Less_Environment::normalizePath(
                            dirname($rooturi . $evald_path)
                        );
                        return [$full_path, $uri];
                    } elseif (file_exists($path . '.less')) {
                        $full_path = Less_Environment::normalizePath(
                            $path . '.less'
                        );
                        $uri = Less_Environment::normalizePath(
                            dirname($rooturi . $evald_path . '.less')
                        );
                        return [$full_path, $uri];
                    }
                }
