...
class Less_Tree_Media extends Less_Tree
{
    ...
    public function compile($env)
    {
        ...
        return !$env->mediaPath
            ? $media->compileTop($env)
            : $media->compileNested($env);
    }
    ...
    public function compileNested($env)
    {
        ...
        foreach ($path as $key => $p) {
            $value =
                $p->features instanceof Less_Tree_Value
                    ? $p->features->value
                    : $p->features;
        ...
        foreach ($permuted as $path) {
            for ($i = 0, $len = count($path); $i < $len; $i++) {
                $path[$i] = Less_Parser::is_method($path[$i], 'toCSS')
                    ? $path[$i]
                    : new Less_Tree_Anonymous($path[$i]);
            }
