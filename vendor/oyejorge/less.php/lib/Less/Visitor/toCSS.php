...
class Less_Visitor_toCSS extends Less_VisitorReplacing
{
    ...
    {
        if (
            isset($directiveNode->currentFileInfo['reference']) &&
        ...
        for ($i = count($rules) - 1; $i >= 0; $i--) {
            ...
            if (
                ...
                if (!isset($ruleCache[$rule->name])) {
                    ...
                    if (
                        $ruleList instanceof Less_Tree_Rule ||
                        $ruleList instanceof Less_Tree_NameValue
                    ) {
                        $ruleList = $ruleCache[$rule->name] = [
                            $ruleCache[$rule->name]->toCSS(),
                        ];
                    }
        ...
        foreach ($groups as $parts) {
            if (count($parts) > 1) {
                ...
                foreach ($parts as $p) {
                    if ($p->merge === '+') {
                        if ($lastSpacedGroup) {
                            $spacedGroups[] = self::toExpression(
                                $lastSpacedGroup
                            );
                        }
                        $lastSpacedGroup = [];
