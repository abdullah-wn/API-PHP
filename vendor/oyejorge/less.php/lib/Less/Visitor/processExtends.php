...
class Less_Visitor_processExtends extends Less_Visitor
{
    ...
    public function run($root)
    {
        ...
        $root->allExtends = $this->doExtendChaining(
            $root->allExtends,
            $root->allExtends
        );
    ...
    private function doExtendChaining(
        $extendsList,
        $extendsListTarget,
        $iterationCount = 0
    ) {
        ...
        for (
            $extendIndex = 0, $extendsList_len = count($extendsList);
            $extendIndex < $extendsList_len;
            $extendIndex++
        ) {
            for (
                $targetExtendIndex = 0;
                $targetExtendIndex < count($extendsListTarget);
                $targetExtendIndex++
            ) {
                ...
                if ($matches) {
                    ...
                    foreach ($extend->selfSelectors as $selfSelector) {
                        ...
                        $newSelector = $this->extendSelector(
                            $matches,
                            $selectorPath,
                            $selfSelector
                        );
                        ...
                        $newExtend = new Less_Tree_Extend(
        ...
        if ($extendsToAdd) {
            ...
            $extendsToAdd = $this->doExtendChaining(
                $extendsToAdd,
                $extendsListTarget,
                $iterationCount + 1
            );
    ...
    protected function visitRuleset($rulesetNode)
    {
        ...
        foreach ($allExtends as $allExtend) {
            for ($pathIndex = 0; $pathIndex < $paths_len; $pathIndex++) {
                ...
                if (
                    isset($rulesetNode->extendOnEveryPath) &&
    ...
    private function ExtendMatch($rulesetNode, $extend, $selectorPath)
    {
        ...
        if ($matches) {
            foreach ($extend->selfSelectors as $selfSelector) {
                $rulesetNode->paths[] = $this->extendSelector(
                    $matches,
                    $selectorPath,
                    $selfSelector
                );
    ...
    private function findMatch($extend, $haystackSelectorPath)
    {
        ...
        for (
            ...
            $haystackSelectorIndex < $haystack_path_len;
            ...
            for (
                ...
                $hackstackElementIndex < $haystack_elements_len;
                    ...
                    if (
                        ...
                        if (
                            ...
                            ($hackstackElementIndex + 1 <
                                $haystack_elements_len ||
                                $haystackSelectorIndex + 1 < $haystack_path_len)
                        ) {
                            $potentialMatch = null;
                        }
                    }
     ...
     */
    private function PotentialMatch(
        $potentialMatch,
        $needleElements,
        $haystackElement,
        $hackstackElementIndex
    ...
    private function isElementValuesEqual($elementValue1, $elementValue2)
    {
        ...
        if ($elementValue1 instanceof Less_Tree_Attribute) {
            return $this->isAttributeValuesEqual(
                $elementValue1,
                $elementValue2
            );
        }
    ...
    private function isSelectorValuesEqual($elementValue1, $elementValue2)
    {
        ...
        if (
            !($elementValue2 instanceof Less_Tree_Selector) ||
            $elementValue1->elements_len !== $elementValue2->elements_len
        ) {
            return false;
        }
        ...
        for ($i = 0; $i < $elementValue1->elements_len; $i++) {
            if (
                $elementValue1->elements[$i]->combinator !==
                $elementValue2->elements[$i]->combinator
            ) {
                if (
                    $i !== 0 ||
                    ($elementValue1->elements[$i]->combinator || ' ') !==
    ...
    private function isAttributeValuesEqual($elementValue1, $elementValue2)
    {
        ...
        $elementValue1 = $elementValue1->value->value
            ? $elementValue1->value->value
            : $elementValue1->value;
        $elementValue2 = $elementValue2->value->value
            ? $elementValue2->value->value
            : $elementValue2->value;

        return $elementValue1 === $elementValue2;
    }

    private function extendSelector(
        $matches,
        $selectorPath,
        $replacementSelector
    ) {
        ...
        for (
            $matchIndex = 0, $matches_len = count($matches);
            $matchIndex < $matches_len;
            $matchIndex++
        ) {
            ...
            if (
                ...
                $last_path->elements = array_merge(
                    $last_path->elements,
                    array_slice(
                        $selectorPath[$currentSelectorPathIndex]->elements,
            ...
            if (
                ...
                $path[$last_key]->elements = array_merge(
                    $path[$last_key]->elements,
                    $newElements
                );
            } else {
                $path = array_merge(
            ...
            if (
                $currentSelectorPathElementIndex >=
                count($selectorPath[$currentSelectorPathIndex]->elements)
            ) {
                $currentSelectorPathElementIndex = 0;
                $currentSelectorPathIndex++;
            }
        ...
        if (
            $currentSelectorPathIndex < $selectorPath_len &&
            $currentSelectorPathElementIndex > 0
        ) {
            $last_path = end($path);
            $last_path->elements = array_merge(
        ...
        $path = array_merge(
            $path,
            array_slice($selectorPath, $currentSelectorPathIndex, $slice_len)
        );
    ...
    {
        $newAllExtends = array_merge(
            $mediaNode->allExtends,
            end($this->allExtendsStack)
        );
        $this->allExtendsStack[] = $this->doExtendChaining(
            $newAllExtends,
            $mediaNode->allExtends
        );
    }
