...
class Less_SourceMap_Generator extends Less_Configurable
{
    ...
    public function __construct(
        ...
        $this->options['sourceMapRootpath'] = $this->fixWindowsPath(
            $this->options['sourceMapRootpath'],
            true
        );
        $this->options['sourceMapBasepath'] = $this->fixWindowsPath(
    ...
    protected function saveMap($file, $content)
    {
        ...
        if (!is_dir($dir)) {
            ...
            throw new Exception(
                sprintf(
                    'The directory "%s" does not exist. Cannot save the source map.',
                    $dir
                )
            );
        }
        ...
        if (file_put_contents($file, $content) === false) {
            throw new Exception(
                sprintf('Cannot save the source map to "%s"', $file)
            );
        }
        return true;
    }
    ...
    protected function generateJson()
    {
        ...
        foreach ($this->sources as $source_uri => $source_filename) {
            $sourceMap['sources'][] = $this->normalizeFilename(
                $source_filename
            );
        }
    ...
    public function generateMappings()
    {
        ...
        foreach ($groupedMap as $lineNumber => $line_map) {
            ...
            foreach ($line_map as $m) {
                $mapEncoded = $this->encoder->encode(
                    $m['generated_column'] - $lastGeneratedColumn
                );
                ...
                if ($m['source_file']) {
                    ...
                    if ($index !== false) {
                        ...
                        $mapEncoded .= $this->encoder->encode(
                            $m['original_line'] - 1 - $lastOriginalLine
                        );
                        $lastOriginalLine = $m['original_line'] - 1;

                        $mapEncoded .= $this->encoder->encode(
                            $m['original_column'] - $lastOriginalColumn
                        );
